...
public sealed class DromScrapeCataloguePagesDecorator(
...
) : IScrapeAdvertisementsHandler
{
    ...
    public async Task Handle(ScrapeAdvertisementsCommand command)
    {
        ...
        while (true)
        {
            ...
            _logger.Information("{Context} page navigated.", nameof(ScrapeAdvertisementsCommand));

            Option<IElementHandle[]> itemsOption = await GetItemsArray(page);
            if (!itemsOption.HasValue)
            {
                _logger.Information("{Context} end reached", nameof(ScrapeAdvertisementsCommand));
                break;
            }
    ...
    )
    {
        ...
        foreach (IElementHandle element in elements)
            data.Add(element, ScrapedAdvertisement.Default());
        return data;
    }
    ...
    )
    {
        foreach (KeyValuePair<IElementHandle, ScrapedAdvertisement> pair in dict)
        {
            ...
            dict[pair.Key] = pair.Value with { PhotoUrls = [.. photos] };
        }
    }
    ...
    private static void InitializeIds(Dictionary<IElementHandle, ScrapedAdvertisement> dict)
    {
        foreach (KeyValuePair<IElementHandle, ScrapedAdvertisement> pair in dict)
        {
            ...
            dict[pair.Key] = pair.Value with { Id = long.Parse(id) };
        }
    }
    ...
    )
    {
        foreach (KeyValuePair<IElementHandle, ScrapedAdvertisement> pair in dict)
        {
            ...
            if (!hrefAttribute.HasValue)
                continue;
            dict[pair.Key] = pair.Value with { SourceUrl = hrefAttribute.Value };
        }
    }
