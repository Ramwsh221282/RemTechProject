...
public sealed record PlainAttributeType(string Label, string Value) : DromTransportAttributeType
{
    public override ScrapedAdvertisement Set(ScrapedAdvertisement advertisement)
    {
        ...
        ScrapedCharacteristic ctx = new() { Name = Label, Value = Value };
        ScrapedCharacteristic[] array = [ctx, .. advertisement.Characteristics];
        clone.Characteristics = array;
        return clone;
    }
}
...
public sealed record PowerAttributeType(int PowerValue) : DromTransportAttributeType
{
    ...
    {
        ScrapedAdvertisement clone = advertisement.Clone();
        ScrapedCharacteristic ctx = new() { Name = "Мощность", Value = $"{PowerValue} л.с." };
        ScrapedCharacteristic[] array = [ctx, .. advertisement.Characteristics];
        clone.Characteristics = array;
        return clone;
    }
...
// public sealed record HasMileageAttributeType(bool HasMileageValue) : DromTransportAttributeType
// {
//     public override ScrapedAdvertisement Set(ScrapedAdvertisement advertisement)
//     {
...
public sealed record MileageAttributeType(string Label, string Marks) : DromTransportAttributeType
{
    ...
    {
        ScrapedAdvertisement clone = advertisement.Clone();
        ScrapedCharacteristic ctx = new() { Name = Label, Value = Marks };
        ScrapedCharacteristic[] array = [ctx, .. advertisement.Characteristics];
        clone.Characteristics = array;
        return clone;
    }
...
// public sealed record SpecialDescriptionAttributeType(bool HasDamage, bool HasDocs) : DromTransportAttributeType
// {
//     public override ScrapedAdvertisement Set(ScrapedAdvertisement advertisement)
//     {
//         return advertisement;
...
public sealed record FrameAttributeType(bool IsVin, string FrameNumber) : DromTransportAttributeType
{
    ...
    {
        ScrapedAdvertisement clone = advertisement.Clone();
        ScrapedCharacteristic ctx = new() { Name = "VIN", Value = FrameNumber.Replace("*", "") };
        ScrapedCharacteristic[] array = [ctx, .. advertisement.Characteristics];
        clone.Characteristics = array;
        return clone;
    }
...
public static class DromTransportAttributeTypeFactory
{
    ...
    {
        ScrapedAdvertisement clone = advertisement.Clone();
        return types.Aggregate(clone, (current, type) => type.Set(current));
    }
    ...
    public static Result<DromTransportAttributeType> Create(JsonElement field)
    {
        ...
        return typeName switch
        {
            ...
            //not null when typeName.IsMatch(DromTransportAttributeType.HasMileage) => AsHasMileageAttributeType(field),
            ...
            //not null when typeName.IsMatch(DromTransportAttributeType.SpecialDescription) => AsSpecialDescriptionAttributeType(field),
    ...
    // private static Result<DromTransportAttributeType> AsHasMileageAttributeType(this JsonElement field)
    // {
    //     Result<JsonElement> payloadResult = field.GetPayload();
    //     if (payloadResult.IsFailure) return payloadResult.Error;
    //     JsonElement payload = payloadResult.Value;
    //     bool hasMileage = payload.GetBoolean();
    //     return new HasMileageAttributeType(hasMileage);
    // }
    ...
    // private static Result<DromTransportAttributeType> AsSpecialDescriptionAttributeType(this JsonElement field)
    // {
    //     Result<JsonElement> payloadResult = field.GetPayload();
    //     if (payloadResult.IsFailure) return payloadResult.Error;
    //     JsonElement payload = payloadResult.Value;
    //     bool hasDamageProperty = payload.TryGetProperty("hasDamage", out JsonElement damageProperty);
    //     if (!hasDamageProperty) return new Error("Damage property is empty");
    //     bool hasDocsProperty = payload.TryGetProperty("hasDocs", out JsonElement docsProperty);
    //     if (!hasDocsProperty) return new Error("Docs Property is empty");
    //     bool hasDamageValue = damageProperty.GetBoolean();
    //     bool hasDocsValue = docsProperty.GetBoolean();
    //     return new SpecialDescriptionAttributeType(hasDamageValue, hasDocsValue);
    // }
